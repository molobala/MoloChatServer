...
public class SecondServeur {
    ...
    private final static List<Traitement> mTraitements = new ArrayList<>();
    ...
    private ServerSocket mServeurSock = null;
    private ServerSocket mVoipSock = null;
    ...
    public void run() {
        System.out.println("En ecoute des clients" + voipThread);
        ...
        private void close() {
            try {
                if (isAuthentificated && mMember != null) {
                    synchronized (LOGS) {
                        LOGS.removeIf(p -> p.second == this.mSocket);
                    }
                }
                mSocket.close();
            } catch (IOException e) {
        ...
        public void run() {
            ...
            try {
                ...
                message.setStatus(MyStandarMessageType.OK);
                {
                    ...
                    n.put("message", "Bienvenu");
                    n.put("exp", spec.getPublicExponent().toString());
                    println(createResponse(0, MyStandarMessageType.EXCHANGE, n));
                }
                ...
                while (mSocket.isConnected()) {
                    ...
                    try {
                        ...
                        if (!isAuthentificated) {
                            ...
                            } else if (command.matches("setRSAKey")) {
                                System.out.println("setRSAKey:" + request);
                                if (request.has("mod") && request.has("exp")) {
                                    BigInteger mod = new BigInteger(request.get("mod").asText());
                                    BigInteger exp = new BigInteger(request.get("exp").asText());
                                    System.out.println("Mod : " + mod);
                                    System.out.println("Exp : " + exp);
                                    clientPubKey = RSAEncryption.KeyManager.readPublicKey(mod, exp);
                                    message.setStatus(MyStandarMessageType.OK);
                                    message.setId(comId);
                                    message.setContent("Done");
                                } else {
                                    message.setContent("Please provide mod and exp");
                                    message.setId(comId);
...
//										System.err.println("MEDIASSS");
                            ...
                            if (command.matches("updateProfil")) {
                                ...
                                if (csarKey != null) {
                                    ObjectNode n = mapper.createObjectNode();
                                    byte[] bytes = csarKey.getBytes();
                                    if (clientPubKey != null) {
                                        bytes = RSAEncryption.encrypt(bytes, clientPubKey);
                                        System.out.println(bytes.length + ":" + new String(Base64.getEncoder().encode(bytes)));
                                        System.out.println("csar" + csarKey);
                                    }
                                    System.out.println("csar: " + csarKey + "; " + clientPubKey);
                                    n.put("key", new String(Base64.getEncoder().encode(bytes)));
                                    println(createResponse(comId, MyStandarMessageType.OK, n));
                                } else {
                                    message.setContent("No key is genereted ");
                                    message.setId(comId);
